...
class OtScrollspy extends HTMLElement {
  ...
  #update() {
    ...
    for (const target of this.targets) {
      if (target.getBoundingClientRect().top <= reference + 1) active = target;
      else break;
    }

    const atEnd = this.root
      ? this.root.scrollTop + this.root.clientHeight >= this.root.scrollHeight - 1
      : window.scrollY + window.innerHeight >= document.documentElement.scrollHeight - 1;
    if (atEnd) active = this.targets.at(-1);
    if (!active || active.id === this.#activeId) return;
