...
import { closeOnOutsidePointer } from './outside-pointer.js';
...
class OtMultiselect extends HTMLElement {
  ...
  get value() { return [...(this.select?.selectedOptions || [])].map(option => option.value); }
  set value(values) {
    const selected = new Set([].concat(values).map(String));
    [...(this.select?.options || [])].forEach(option => { option.selected = selected.has(option.value); });
    this.#sync();
  }
  get options() { return [...(this.list?.querySelectorAll('[role="option"]') || [])]; }
  get enabledOptions() { return this.options.filter(option => option.getAttribute('aria-disabled') !== 'true'); }
  ...
  toggle() { this.list.hidden ? this.open() : this.close(); }
  ...
  #connect() {
    ...
    this.list.addEventListener('click', event => {
      ...
      if (option && option.getAttribute('aria-disabled') !== 'true') this.#choose(option);
    }, { signal });
  ...
  #build() {
    ...
    const id = uid('oatbase-multiselect');
  ...
  #keydown(event) {
    if (['ArrowDown', 'ArrowUp', 'Home', 'End'].includes(event.key)) {
      ...
      event.preventDefault();
      if (this.list.hidden) this.open();
      else this.#choose(this.enabledOptions[this.#active]);
