...
class OtDataTable extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.table.addEventListener('change', event => {
      if (event.target.matches('[data-table-select-row]')) this.#selectionChanged(event.target);
      else if (event.target.matches('[data-table-select-all]')) this.#toggleVisible(event.target.checked);
    }, { signal });
  ...
  get visibleRows() { return this.rows.filter(row => !row.hidden); }
  get selectedRows() { return this.rows.filter(row => row.querySelector('[data-table-select-row]:checked')); }

  refresh() {
    this.rows.forEach((row, index) => {
      if (!this.#order.has(row)) this.#order.set(row, index);
    });
    this.#filter(false);
  }
  ...
  #sortBy(header, index) {
    const direction = this.#sort.index !== index || this.#sort.direction === 'none'
      ? 'ascending'
      : this.#sort.direction === 'ascending' ? 'descending' : 'none';
    this.#sort = { index, direction };
    [...this.table.tHead?.rows[0]?.cells || []]
      .filter(cell => cell.hasAttribute('data-sort'))
      .forEach(cell => cell.setAttribute('aria-sort', cell === header ? direction : 'none'));

    const type = header.dataset.sort || 'text';
    const rows = [...this.rows].sort((left, right) => {
      if (direction === 'none') return this.#order.get(left) - this.#order.get(right);
  ...
  #filter(announce = true) {
    ...
    if (this.status) this.status.value = query ? `${visible} of ${total} rows` : `${total} rows`;
    if (this.empty) this.empty.hidden = visible > 0;
    this.#syncSelection();
    if (announce) emit(this, 'oatbase:filter', { query, visible, total });
  }
  ...
  #syncSelection() {
    ...
    const visibleInputs = this.visibleRows
      .map(row => row.querySelector('[data-table-select-row]:not(:disabled)'))
      .filter(Boolean);
    ...
    if (this.selectedStatus) this.selectedStatus.value = `${this.selectedRows.length} selected`;
  }
