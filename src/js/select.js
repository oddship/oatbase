...
class OtSelect extends HTMLElement {
  ...
  open() {
    ...
    if (active) this.button.setAttribute('aria-activedescendant', active.id);
  }
  ...
  #build() {
    ...
    [...this.select.options].forEach((source, index) => {
      if (!source.value && source.disabled) return;
  ...
  #onKeydown(event) {
    if (['ArrowDown', 'ArrowUp', 'Home', 'End'].includes(event.key)) {
      ...
      event.preventDefault();
      if (this.list.hidden) this.open();
      else if (this.#active >= 0) this.#choose(this.enabledOptions[this.#active]);
