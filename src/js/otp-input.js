...
class OtOtp extends HTMLElement {
  ...
  connectedCallback() {
    ...
    if (this.input.maxLength < 1) this.input.maxLength = this.length;
    ...
    if (!this.slots.isConnected) this.append(this.slots);
  ...
  handleEvent(event) {
    if (event.type === 'reset') setTimeout(() => this.#render(false));
  ...
  #render(fromInput) {
    ...
    if (fromInput && complete && !this.#complete) emit(this, 'oatbase:complete', { value });
