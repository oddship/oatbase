...
class OtCombobox extends HTMLElement {
  ...
  #filter() {
    ...
    if (this.create) {
      const exactMatch = sourceOptions.some(option => {
        ...
        return option.textContent.trim().toLocaleLowerCase() === query
          || source?.value.toLocaleLowerCase() === query;
      });
  ...
  #choose(option) {
    ...
    if (option.hasAttribute('data-create')) {
      ...
      if (!value) return;
      const source = new Option(value, value, true, true);
      source.dataset.custom = '';
      this.select.append(source);
      const presentation = this.#appendOption(source, source.index, this.create);
      emit(this, 'oatbase:create', { value, option: source });
      option = presentation;
    }
    const source = option._source;
  ...
  #appendOption(source, index, before = null) {
    if (!source.value && source.disabled) return null;
