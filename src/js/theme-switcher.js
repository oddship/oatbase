...
class OtThemeSwitcher extends HTMLElement {
  ...
  connectedCallback() {
    ...
    try { initial = localStorage.getItem(this.key) || initial; } catch { /* Use the declarative default. */ }
  ...
  #target() {
    if (!this.dataset.target) return this.ownerDocument.documentElement;
    try { return this.ownerDocument.querySelector(this.dataset.target); }
    catch { return null; }
  }
  ...
  #apply(notify) {
    ...
    if (target) {
      ...
      if (this.#theme === 'system') target.removeAttribute('data-theme');
      else target.dataset.theme = this.#theme;
    }
    try { localStorage.setItem(this.key, this.#theme); } catch { /* Theme still applies for this page view. */ }
    ...
    if (this.toggleButton) {
      ...
      if (this.toggleContents.length) {
        this.toggleContents.forEach(content => { content.hidden = content.dataset.themeContent !== this.#theme; });
    ...
    if (notify) emit(this, 'change', { value: this.#theme });
  }
}

define('ot-theme-switcher', OtThemeSwitcher);
export { OtThemeSwitcher };
